...
const PANTRY_FIRST_RE =
  /\b(enak\s+apa|masak\s+apa|what\s+can\s+i\s+cook|with\s+what\s+i\s+have|cuma\s+punya|cuman\s+punya|hanya\s+punya|only\s+have|bahan[- ]?bahan|dari\s+pantry|from\s+(my\s+)?pantry|recommend\s+(a\s+)?(menu|dish|meal))\b/i;

const KNOWN_DISH_RE =
  /\b(masak|cook|buat|make|recipe\s+for)\b.+\b(ayam|semur|rendang|soto|nasi|steak|sop|soup|curry|gulai)\b/i;
...
- known_dish: user names a specific dish
